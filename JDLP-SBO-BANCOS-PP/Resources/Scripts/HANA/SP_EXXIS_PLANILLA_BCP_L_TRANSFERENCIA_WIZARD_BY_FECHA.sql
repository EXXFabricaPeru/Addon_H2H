CREATE PROCEDURE "SP_EXXIS_PLANILLA_BCP_L_TRANSFERENCIA_WIZARD_BY_FECHA"
(
    IN FI NVARCHAR(100),
    IN FF NVARCHAR(100)
)
...
BEGIN
    ...
    WHERE
        T0."DocType" = 'S'
        AND T0."Canceled" != 'Y'
        AND T0."DocDate" BETWEEN TO_DATE(:FI, 'YYYY-MM-DD') AND TO_DATE(:FF, 'YYYY-MM-DD')
        AND (
        ...
        AND T0."DocEntry" NOT IN (SELECT "U_DocEntry" FROM "@EXX_BCP_TRAN_OVPM");
END;
